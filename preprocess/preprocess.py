    ...
    with open(file_path, "r") as file:
        for line in file:
            if line.startswith("!Sample_geo_accession"):
                geo_accessions = [
                    id_.strip('"') for id_ in line.strip().split("\t")[1:]
                ]  # Skip the first entry
                print(f"GEO Accessions: {geo_accessions}")
            elif (
                line.startswith("!Sample_characteristics_ch1") and "age" in line.lower()
            ):
                age_matches = re.findall(
                    r"age(?: \(y\))?:\s*(\d+)", line, flags=re.IGNORECASE
                )
...
def process_with_dask(file_path):
    ...
    try:
        ...
        numeric_cols = ddf.select_dtypes(
            include=["number"]
        ...
        print(
            f"Processed DataFrame: {persist_ddf.npartitions} partitions, columns: {persist_ddf.columns}"
        )
        del ddf
        return persist_ddf
...
def preprocess_and_combine(
    folder_27k, folder_450k, output_27k, output_450k, final_output
):
    ...
    with ProgressBar():
        common_probes = (
            dd.merge(
                combined_27k_ddf_index,
                combined_450k_ddf_index,
                how="inner",
                left_on="probe_27k",
                right_on="probe_450k",
            )
    ...
    combined_data_ddf = dd.concat(
        [combined_27k_ddf, combined_450k_ddf], axis=1
    )

    # Ensure valid sample names and alignment
    valid_samples = [
        sample.strip('"').strip() for sample in all_ages["Sample"].tolist()
    ]

    # Transpose data
    aligned_data_ddf = combined_data_ddf[valid_samples].T
    aligned_data_ddf["Age"] = all_ages.set_index("Sample").loc[
        aligned_data_ddf.index, "Age"
    ]
